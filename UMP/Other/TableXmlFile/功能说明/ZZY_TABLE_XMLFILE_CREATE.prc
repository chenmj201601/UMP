...
begin
   ...
   /* if (substr(in_table_name,1,4)='T_00') or (in_table_name='T_51_001') or (substr(in_table_name,1,4)='T_99') or (substr(in_table_name,1,4)='T_25') or (substr(in_table_name,1,4)='T_24') then
    ...
    if instr(in_table_name,'T_44_0')=0 then
      v_in_table_name:=substr(in_table_name,1,8);
    else
      v_in_table_name:=in_table_name;
    end if;

    v_xmlstring:='<TableDefine P01="'||v_in_table_name||'" P02="8.03.001" P03="0" P04="'||v_token||'" P05="'||v_Partition_flag||'" P06="'||v_table_comment||'" P07="备用">';
    ...
    v_execsql := 'select a.column_name,
                  ...
                  b.comments,
                  CASE WHEN LENGTH(A.column_name)=4 THEN  to_number(SUBSTR(A.column_name,2,3))
                       WHEN LENGTH(A.column_name)=5 THEN  10000+to_number(SUBSTR(A.column_name,2,4))
                        END  AS ORDERSID
    ...
    if substr(in_table_name,1,5)='T_44_' then
       ...
       v_execsql:=v_execsql||' order by  ORDERSID';
    end if;
    ...
    loop
      fetch cur_cloumn into v_column_name,v_data_type,v_data_width,v_data_dec,v_isnull,v_comment,v_ORDERSID;
