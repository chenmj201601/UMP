...
namespace UMPS3103
{
    ...
    public partial class QueryRecordInTask 
    {
     ...
     //   public List<GetKeyWordInfo> getKeyWordInfo;
    //    public List<ObjectItemTask> mListSelectedObjects_KeyWords;
        ...
        /// 异常分数
        /// </summary>
        private List<ABCD_OrgSkillGroup> ListABSABCDItem;
        /// <summary>
        /// 亊后处理时长异常
        /// </summary>
        private List<ABCD_OrgSkillGroup> ListAFSABCDItem;
        /// <summary>
        /// 坐席/客人讲话时长比例异常
        /// </summary>
        /// 
        private List<ABCD_OrgSkillGroup> ListAGSABCDItem;
        ...
        public QueryRecordInTask()
        {
          ...
          //  getKeyWordInfo = new List<GetKeyWordInfo>();
            ...
            ListABSABCDItem = new List<ABCD_OrgSkillGroup>();
            ListAFSABCDItem = new List<ABCD_OrgSkillGroup>();
            ListAGSABCDItem = new List<ABCD_OrgSkillGroup>();
        ...
        void QueryRecordInTask_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                temp.Add("3110000000000000005");//坐席/客人讲话时长比例异常
                temp.Add("3110000000000000007");//事后处理时长异常
                temp.Add("3110000000000000008");//异常分数
                InitABCDItem(temp);
        ...
        private void ChangeLanguage()
        {
            ...
            chAbnormalScores.Content = CurrentApp.GetLanguageInfo("3103T00185", "ExceptionScore");
            abSGood.Content = CurrentApp.GetLanguageInfo("3103T00172", "Normal");
            abSBad.Content = CurrentApp.GetLanguageInfo("3103T00173", "Abnormal");
            abSDAll.Content = CurrentApp.GetLanguageInfo("3103T00131", "All");
            chAfterEventProcessing.Content = CurrentApp.GetLanguageInfo("3103T00186", "AfterDealDurationExcept");
            aEPGood.Content = CurrentApp.GetLanguageInfo("3103T00172", "Normal");
            aEPBad.Content = CurrentApp.GetLanguageInfo("3103T00173", "Abnormal");
            aEPDAll.Content = CurrentApp.GetLanguageInfo("3103T00131", "All");
            chSeatAgentSpeechAnomaly.Content = CurrentApp.GetLanguageInfo("3103T00187", "ACSpeExceptProportion");
            asASGood.Content = CurrentApp.GetLanguageInfo("3103T00172", "Normal");
            asASBad.Content = CurrentApp.GetLanguageInfo("3103T00173", "Abnormal");
            asASDAll.Content = CurrentApp.GetLanguageInfo("3103T00131", "All");
            chbKeyWord.Content = CurrentApp.GetLanguageInfo("3103T00188", "KeyWord");
        ...
        private bool CreateQueryCondition() 
        {
            ...
            try
            {
                ...
                //KeyWord关键词
                if (chbKeyWord.IsChecked.Equals(true))
                {                  
                    string strValue01 = string.Empty;
                    string strValue02 = string.Empty;
                    if (keyWordList1.mListGeyKeyWordInfo != null && keyWordList1.mListGeyKeyWordInfo.Count > 0)
                    {
                        for (int i = 0; i < keyWordList1.mListGeyKeyWordInfo.Count; i++)
                        {
                            var item = keyWordList1.mListGeyKeyWordInfo[i];
                            strValue01 += string.Format("'{0}';", item.Description);                                        
                        }
                        for (int j = 0; j < keyWordList1.mListKeywordInfos.Count; j++)
                        {
                            var info = keyWordList1.mListKeywordInfos[j];
                            strValue02 += string.Format("'{0}',", info.Content);
                        }          
                        strValue01 = strValue01.TrimEnd(';');
                        strValue02 = strValue02.TrimEnd(',');
                        txtKeyWord.Text = strValue01;
                    }
                    if (chbKeyWord.IsChecked.Equals(true) && txtKeyWord.Text.Trim(' ').Length > 0)
                    {
                        querycontionDetail = new QueryConditionDetail();
                        querycontionDetail.ConditionItemID = S3103Consts.CON_KEYWORD;
                        querycontionDetail.IsEnable = true;
                        querycontionDetail.IsLike = true;
                        txtKeyWord.Text = strValue01;
                        querycontionDetail.Value01 = txtKeyWord.Text.Trim(' ').ToString();
                        querycontionDetail.Value02 = strValue02;
                        mListQueryConditionDetails.Add(querycontionDetail);
                    }
                    else
                    {
                        ShowInformation(CurrentApp.GetLanguageInfo("3103T00189", "Please select key words."));
                        return false;
                    }
                }
                ...
                //ABCD查询项--异常分数
                if (chAbnormalScores.IsChecked.Equals(true)) 
                {
                    querycontionDetail = new QueryConditionDetail();
                    querycontionDetail.ConditionItemID = S3103Consts.CON_ExceptionScore;
                    querycontionDetail.IsEnable = true;
                    querycontionDetail.IsLike = false;
                    if (abSGood.IsChecked.Equals(true)) //正常
                    {
                        querycontionDetail.Value01 = "1";
                    }
                    else if (abSBad.IsChecked.Equals(true)) //异常
                    {

                        querycontionDetail.Value01 = "2";
                    }
                    else if (abSDAll.IsChecked.Equals(true)) //全部
                    {
                        querycontionDetail.Value01 = "0";
                    }
                    if (string.IsNullOrWhiteSpace(querycontionDetail.Value01)) //如果没有选择哪一项，默认勾选全部
                    {
                        abSDAll.IsChecked = true;
                        querycontionDetail.Value01 = "0";
                    }
                    if (cbAbnormalScores.SelectedIndex < 0) 
                    {
                        ShowInformation(CurrentApp.GetLanguageInfo("3103T00171", "Select One Org"));
                        return false;
                    }
                    querycontionDetail.Value02 = ListABSABCDItem[cbAbnormalScores.SelectedIndex].InColumn.ToString();
                    querycontionDetail.Value03 = ListABSABCDItem[cbAbnormalScores.SelectedIndex].ManageAgent;
                    querycontionDetail.Value04 = string.Format("AbnS");
                    mListQueryConditionDetails.Add(querycontionDetail);
                }
                //ABCD查询项--亊后处理时长异常
                if (chAfterEventProcessing.IsChecked.Equals(true)) 
                {
                    querycontionDetail = new QueryConditionDetail();
                    querycontionDetail.ConditionItemID = S3103Consts.CON_AfterDealDurationExcept;
                    querycontionDetail.IsEnable = true;
                    querycontionDetail.IsLike = false;
                    if (aEPGood.IsChecked.Equals(true))//正常
                    {
                        querycontionDetail.Value01 = "1";
                    }
                    else if (aEPBad.IsChecked.Equals(true)) 
                    {
                        querycontionDetail.Value01 = "2";
                    }
                    else if (aEPDAll.IsChecked.Equals(true)) 
                    {
                        querycontionDetail.Value01 = "0";
                    }
                    if (string.IsNullOrWhiteSpace(querycontionDetail.Value01)) 
                    {
                        aEPDAll.IsChecked = true;
                        querycontionDetail.Value01 = "0";
                    }
                    if (cbAfterEventProcessing.SelectedIndex < 0) 
                    {
                        ShowInformation(CurrentApp.GetLanguageInfo("3103T00171", "Select One Org"));
                        return false;
                    }
                    querycontionDetail.Value02 = ListAFSABCDItem[cbAfterEventProcessing.SelectedIndex].InColumn.ToString();
                    querycontionDetail.Value03 = ListAFSABCDItem[cbAfterEventProcessing.SelectedIndex].ManageAgent;
                    querycontionDetail.Value04 = string.Format("AfEP");
                    mListQueryConditionDetails.Add(querycontionDetail);
                }
                //ABCD查询项--坐席/客人讲话时长比例异常
                if (chSeatAgentSpeechAnomaly.IsChecked.Equals(true)) 
                {
                    querycontionDetail = new QueryConditionDetail();
                    querycontionDetail.ConditionItemID = S3103Consts.CON_ACSpeExceptProportion;
                    querycontionDetail.IsEnable = true;
                    querycontionDetail.IsLike = false;
                    if (asASGood.IsChecked.Equals(true)) 
                    {
                        querycontionDetail.Value01 = "1";
                    }
                    else if (asASBad.IsChecked.Equals(true)) 
                    {
                        querycontionDetail.Value01 = "2";
                    }
                    else if (asASDAll.IsChecked.Equals(true)) 
                    {
                        querycontionDetail.Value01 = "0";
                    }
                    if (string.IsNullOrWhiteSpace(querycontionDetail.Value01)) 
                    {
                        asASDAll.IsChecked = true;
                        querycontionDetail.Value01 = "0";
                    }
                    if (cbSeatAgentSpeechAnomaly.SelectedIndex < 0) 
                    {
                        ShowInformation(CurrentApp.GetLanguageInfo("3103T00171", "Select One Org"));
                        return false;
                    }
                    querycontionDetail.Value02 = ListAGSABCDItem[cbSeatAgentSpeechAnomaly.SelectedIndex].InColumn.ToString();
                    querycontionDetail.Value03 = ListAGSABCDItem[cbSeatAgentSpeechAnomaly.SelectedIndex].ManageAgent;
                    querycontionDetail.Value04 = string.Format("SeASP");
                    mListQueryConditionDetails.Add(querycontionDetail);
                }
        ...
        public void InitABCDItem(List<string> OperationList)
        {
            try
            {
             ...
             //  OperationList.Sort();
                ...
                //异常分数
                ListABSABCDItem = InitABCDQuery(OperationList[6]);
                if (ListABSABCDItem.Count() > 0) 
                {
                    LogStr = string.Empty;
                    for (int i = 0; i < ListABSABCDItem.Count(); i++) 
                    {
                        tempBox = new ComboBoxItem();
                        tempBox.Content = ListABSABCDItem[i].OrgSkillGroupName;
                        LogStr += string.Format("{0}--{1}\t", ListABSABCDItem[i].OrgSkillGroupName, ListABSABCDItem[i].OrgSkillGroupID);
                        cbAbnormalScores.Items.Add(tempBox);
                    }
                    CurrentApp.WriteLog("异常分数",string.Format("{0}",LogStr));
                }
                //亊后处理时长异常
                ListAFSABCDItem = InitABCDQuery(OperationList[5]);
                if (ListAFSABCDItem.Count() > 0)
                {
                    LogStr = string.Empty;
                    for (int i = 0; i < ListAFSABCDItem.Count(); i++)
                    {
                        tempBox = new ComboBoxItem();
                        tempBox.Content = ListAFSABCDItem[i].OrgSkillGroupName;
                        LogStr += string.Format("{0}--{1}\t", ListAFSABCDItem[i].OrgSkillGroupName, ListAFSABCDItem[i].OrgSkillGroupID);
                        cbAfterEventProcessing.Items.Add(tempBox);
                    }
                    CurrentApp.WriteLog("亊后处理时长异常", string.Format("{0}", LogStr));
                }
                //坐席/客人讲话时长比例异常
                ListAGSABCDItem = InitABCDQuery(OperationList[4]);
                if (ListAGSABCDItem.Count() > 0)
                {
                    LogStr = string.Empty;
                    for (int i = 0; i < ListAGSABCDItem.Count(); i++)
                    {
                        tempBox = new ComboBoxItem();
                        tempBox.Content = ListAGSABCDItem[i].OrgSkillGroupName;
                        LogStr += string.Format("{0}--{1}\t", ListAGSABCDItem[i].OrgSkillGroupName, ListAGSABCDItem[i].OrgSkillGroupID);
                        cbSeatAgentSpeechAnomaly.Items.Add(tempBox);
                    }
                    CurrentApp.WriteLog("坐席/客人讲话时长比例异常", string.Format("{0}", LogStr));
                }
            }
            catch (Exception ex)
            {
