...
namespace UMPS2101
{
    ...
    {
        const string ConditionTypePigeonhole = "1";
        const string ConditionTypeBackups = "2";
        const string ConditionTypeBackToDelete = "3";
        //private bool bChangeParam = false;
        ...
        {
            int index = Convert.ToInt16(item.StrategyType);
            if (index == 1)
                index = 0;
            if (index == 3 )
                index = 1;

            cmbsttype.SelectedIndex = index;
        ...
        public void RemoveCondition(UC_ConditionTextbox uc)
        {
            try
            {
                int count = SPAllParameters.Children.Count;
                if (count > 0)
                {
                    SPAllParameters.Children.Remove(uc);
                }
                count = SPAllParameters.Children.Count;
                if (count <= 0)
                {
                    CreateUC();
                }
        ...
        private void BindLogical()
        {
            ...
            mlststrategy.Add(new CStrategyType() { ID = 3, Show = CurrentApp.GetLanguageInfo("2101T00023", "Delete Strategy") });
        }
        ...
        private void AddFilterStrategy()
        {
            try
            {
                ...
                DateTime dt1=Convert.ToDateTime(DateStart.Text);
                DateTime dt2=Convert.ToDateTime(DateEnd.Text);
               // TimeSpan s = dt1-dt2;
                ...
                {
                    CurrentApp.ShowInfoMessage(CurrentApp.GetLanguageInfo("2101T00032", "DateStartTime < DateStopTime"));
                    return;
                }
                ...
                switch (allfd.StrategyType)
                {
                    case ConditionTypePigeonhole:
                        allfd.StrategyCode = GetSerialID("257");
                        break;
                    case ConditionTypeBackups:
                        allfd.StrategyCode = GetSerialID("258");
                        break;
                    case ConditionTypeBackToDelete:
                        allfd.StrategyCode = GetSerialID("259");
                        break;
                }
        ...
        {
            AllFilterData item = LvStrategy.SelectedItem as AllFilterData;
            AllFilterData oldData = new AllFilterData();
            oldData = LvStrategy.SelectedItem as AllFilterData;
            if (item != null)
            {
                ...
                else 
                {
                    ...
                    if (CompareFilterData( oldData, allfd ))
                    {
                        return;
                    }
                    ...
                    webRequest.Data = optReturn.Data.ToString();
                    webRequest.ListData.Add("0");//1 修改策略
                    //Service21011Client client = new Service21011Client();
                    Service21011Client client = new Service21011Client(WebHelper.CreateBasicHttpBinding(CurrentApp.Session), WebHelper.CreateEndpointAddress(CurrentApp.Session.AppServerInfo, "Service21011"));
                    //WebHelper.SetServiceClient(client);
                    WebReturn webReturn = client.DoOperation(webRequest);
                    ...
                    else
                    {
                        ...
                        //Service21011Client client = new Service21011Client();
                        client = new Service21011Client(WebHelper.CreateBasicHttpBinding(CurrentApp.Session), WebHelper.CreateEndpointAddress(CurrentApp.Session.AppServerInfo, "Service21011"));
                        //WebHelper.SetServiceClient(client);
                        ...
                        {
                            strLog = string.Format("{0}{1}{2}", CurrentApp.Session.UserInfo.UserName, Utils.FormatOptLogString(string.Format("FO2101003")), item.StrategyName);
                            CurrentApp.WriteOperationLog("2101", ConstValue.OPT_RESULT_SUCCESS, strLog);
                        }
        ...
        private bool CompareFilterData(AllFilterData firstData, AllFilterData secondData)
        {
            string firstdata = ParamString(firstData);
            string secdata = ParamString(secondData);
            return string.Equals(firstdata, secdata);
        }

        private string ParamString(AllFilterData data)
        {
            string strData = string.Format("{0}{1}{2}{3}{4}{5}{6}{7}", data.FilterType
                , data.StrategyType
                , data.StrategyCode
                , data.StrategyName
                , data.IsValid
                ...
                , data.DateEnd);
            var strTemp = strData;
            foreach (var first in data.listFilterCondition)
            {
                strData = string.Format("{0}{1}{2}{3}{4}{5}{6}{7}", strTemp
                , first.StrategyCode
                , first.FilterTarget
                , first.ConditionName
                , first.Operator
