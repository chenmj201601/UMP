...
namespace UMPService05
{
    ...
    public partial class UMPService05 : ServiceBase
    {
        ...
        /// T31_023任务中配置的关键词条件
        /// </summary>
        public string IKeyWordContent=string.Empty;
        #endregion
        ...
        private void GetTaskList(string LStrRentToken)
        {
            try
            {
                ...
                {
                    LStrDynamicSQL = string.Format("SELECT T.*,F.C001 F1,F.C002 F2,F.C003 F3,F.C004 F4,F.C005 F5,F.C006 F6,F.C007 F7,F.C008 F8,F.C009 F9,F.C010 F10,F.C011 F11,F.C012 F12,F.C013 F13,F.C014 F14,F.C015 F15,F.C016 F16,F.C017 F17,F.C018 F18,F.C019 F19,F.C020 F20,F.C021 F21,F.C022 F22,F.C023 F23,F.C024 F24,F.C025 F25,F.C033 F33,S.C001 S1,S.C002 S2,S.C003 S3,S.C004 S4,S.C005 S5,S.C006 S6,S.C007 S7,S.C008 S8,S.C009 S9,S.C010 S10,S.C011 S11,S.C012 S12,S.C013 S13,S.C014 S14,S.C015 S15,S.C016 S16,S.C017 S17 FROM T_31_023_{0} T LEFT JOIN T_31_024_{0} F ON T.C013=F.C001 LEFT JOIN T_31_026_{0} S ON T.C014=S.C001",
                        LStrRentToken);
                ...
                foreach (DataRow LDataRow in LDatabaseOperationReturn.DataSetReturn.Tables[0].Rows)
                {
                    ...
                    mat.KeyWordContentOne = Convert2String(LDataRow["C021"]);
                    mat.KeyWordContentTwo = Convert2String(LDataRow["C022"]);
                    mat.KeyWordContentThree = Convert2String(LDataRow["C023"]);
                    mat.TemplateID = Convert2Long(LDataRow["C027"]);
                    ...
                    mqp.BookMarkStr = Convert2String(LDataRow["F25"]);
        ...
        private void SaveTask(T_AutoTaskSet task,UserTasksInfoShow newTask, List<string> listctrolqa, List<RecordInfo> lstRecordInfoItem, string LStrRentToken, bool IsLogicPartitionTables)
        {
            ...
            try
            {
                if (IIntDatabaseType == 2)
                {
                    ...
                    DbParameter[] mssqlParameters =
                        {
                            ...
                            MssqlOperation.GetDbParameter("@ainparam19",MssqlDataType.NVarchar,50),
                    ...
                    mssqlParameters[17].Value = task.TemplateID;

                    mssqlParameters[18].Value = serialID;
                    mssqlParameters[19].Value = errNum;
                    mssqlParameters[20].Value = errMsg;
                    ...
                    if (mssqlParameters[19].Value.ToString() != "0")
                    {
                        optReturn.Message = string.Format("{0}\t{1}", mssqlParameters[20].Value, mssqlParameters[14].Value);
                        FileLog.WriteError("SaveTask()2", string.Format("{0}\t{1}", mssqlParameters[20].Value, mssqlParameters[14].Value));
                        return;
                    }
                    else
                    {
                        taskID = Convert.ToInt64(mssqlParameters[18].Value.ToString());
                ...
                else if (IIntDatabaseType == 3)
                {
                    ...
                    DbParameter[] dbParameters =
                        {
                            ...
                            OracleOperation.GetDbParameter("ainparam19",OracleDataType.Nvarchar2,50),
                    ...
                    dbParameters[17].Value = task.TemplateID;


                    dbParameters[18].Value = serialID;
                    dbParameters[19].Value = errNum;
                    dbParameters[20].Value = errMsg;
                    ...
                    if (dbParameters[19].Value.ToString() != "0")
                    {
                        optReturn.Message = string.Format("{0}\t{1}", dbParameters[120].Value, dbParameters[14].Value);
                        FileLog.WriteError("SaveTask() 2", string.Format("{0}\t{1}", dbParameters[20].Value, dbParameters[14].Value));
                        return;
                    }
                    else
                    {
                        taskID = Convert.ToInt64(dbParameters[18].Value.ToString());
        ...
        private string GetConditionStrAll(T_AutoTaskSet task, string tablename, string LStrRentToken, string strStartData, string strEndData, string agents,string table54)
        {
            try
            {
                ...
                StringBuilder strsb = new StringBuilder();
                bool hasABCD = qp.ABCDCondition != "";
                IKeyWordContent = task.KeyWordContentOne + task.KeyWordContentTwo + task.KeyWordContentThree;
                IKeyWordContent = GetKeyWordContentStr(IKeyWordContent);
                ...
                if (IIntDatabaseType == 2)
                {
                    ...
                    //关键词
                    if (!string.IsNullOrWhiteSpace(IKeyWordContent))
                    {
                        strsb.Append(string.Format(" AND T21.C002 IN(SELECT DISTINCT C002 FROM T_51_009_{0} WhERE C008 IN({1}))", LStrRentToken, IKeyWordContent));
                    }
                    //bookmark
                    if (!string.IsNullOrWhiteSpace(qp.BookMarkStr))
                    {
                        strsb.Append(string.Format(" AND T21.C073 like '{0}'", qp.BookMarkStr));
                    }
                    //不查询录屏
        ...
        private string GetKeyWordContentStr(string kwstr)
        {
            string keywordStr = string.Empty;
            string[] items = kwstr.Split(new[] { ';' },StringSplitOptions.None);
            foreach (string item in items)
            {
                keywordStr += string.Format("'{0}',",item);
            }
            keywordStr = keywordStr.TrimEnd(',');
            return keywordStr;
        }
        #endregion
