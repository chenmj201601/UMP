 ...
 * 
 * ***启动***
 *      与一般的UMP服务一样，MonitorServer在启动的过程分为以下几个部分：
 * 1、创建日志处理器（仅控制台模式需要）
 * 2、初始化，初始化执行以下操作：
 * （1）加载配置选项
 * （2）应用配置选项
...
namespace UMPService04
{
    ...
    public class MonitorServer : IEncryptable, INetServer
    {
        ...
        private void LoadResourceInfos()
        {
            try
            {
                    ...
                    case 2:
                        strSql =
                            string.Format(
                                "SELECT * FROM T_11_101_{0} WHERE C001 > {1} AND C001 < {2} AND C002 = 1 ORDER BY C001, C002",
                                rentToken,
                                RecoverServerInfo.RESOURCE_RECOVERSERVER * 10000000000000000,
                                (RecoverServerInfo.RESOURCE_RECOVERSERVER + 1) * 10000000000000000);
                        optReturn = MssqlOperation.GetDataSet(strConn, strSql);
                        if (!optReturn.Result)
                        {
                            OnDebug(LogMode.Error,
                ...
                for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    RecoverServerInfo recover = new RecoverServerInfo();
                    recover.ObjType = RecoverServerInfo.RESOURCE_RECOVERSERVER;
                    long objID = Convert.ToInt64(dr["C001"]);
                    recover.ObjID = objID;

                    var temp = listConfigInfos.FirstOrDefault(r => r.ObjID == recover.ObjID);
                    if (temp != null)
                    {
                        listConfigInfos.Remove(temp);
                    }
                    recoverCount++;
                    listConfigInfos.Add(recover);
                }

                #endregion


                #region 获取RecoverServer的配置信息

                for (int i = 0; i < listConfigInfos.Count; i++)
                {
                    var resource = listConfigInfos[i];
                    if (resource.ObjType != RecoverServerInfo.RESOURCE_RECOVERSERVER) { continue; }
                    var recover = resource as RecoverServerInfo;
                    ...
                    switch (dbInfo.TypeID)
                    {
                        case 2:
                            strSql =
                                string.Format(
                                    "SELECT * FROM T_11_101_{0} WHERE C001 ={1} and (C002 = 1 OR C002 = 2 OR C002 = 91) ORDER BY C001, C002",
                                    rentToken,
                                    recover.ObjID);
                            optReturn = MssqlOperation.GetDataSet(strConn, strSql);
                            if (!optReturn.Result)
                    ...
                    for (int j = 0; j < objDataSet.Tables[0].Rows.Count; j++)
                    {
                        ...
                        if (row == 1)
                        {
                            ...
                            string strHostAddress = dr["C017"].ToString();
                            strHostAddress = DecodeEncryptValue(strHostAddress);
                            recover.HostAddress = strHostAddress;
                            string strHostName = dr["C018"].ToString();
                            strHostName = DecodeEncryptValue(strHostName);
                            recover.HostName = strHostName;
                            string strPort = dr["C019"].ToString();
                            strPort = DecodeEncryptValue(strPort);
                            int intValue;
                            if (int.TryParse(strPort, out intValue))
                            ...
                            recover.HostPort = intValue;
                        }
                        if (row == 91)
                        {
                            recover.Continent = dr["C011"].ToString();
                            recover.Country = dr["C012"].ToString();
                        }
                    }
                }
                ...
                OnDebug(LogMode.Info,
                    string.Format(
                        "LoadResourceInfos end.\tDECCount:{0};\tVoiceCount:{1};\tScreenCount:{2};\tIsaServerCount:{3};\tVoiceChanCount:{4};\tScreenChanCount:{5};\tRecoverCount:{6}",
                        decCount,
                        voiceCount,
                        screenCount,
                        isaCount,
        ...
        void DecConnector_ServerConnectedEvent(bool isConnected, string msg)
        {
            try
            {
                if (isConnected)
                {
                    ...
                    #region 订阅RecoverServer发布的所有消息

                    //订阅RecoverServer发布所有消息
                    mask = new MessageString();
                    mask.SourceModule = 0xffff;      //模块掩码
                    mask.SourceNumber = 0x0000;
                    mask.TargetModule = 0x0000;
                    ...
                    mask.Number = 0x0000;
                    message = new MessageString();
                    message.SourceModule = 0x152b;      //由RecoverServer发布的所有消息
                    message.SourceNumber = 0x0000;
                    message.TargetModule = 0x0000;
                    message.TargetNumber = 0x0000;
                    message.LargeType = 0x0000;
            ...
            try
            {
                ...
                switch (strMessage)
                {
                    ...
                    case DECMessageHelper.MSG_VOC_CHANNELCONNECTED:
                        strVoiceID = mDECMessageHelper.GetVoiceIDValue(strData);
                        strChannelID = mDECMessageHelper.GetChannelIDValue(strData);
                        OnDebug(LogMode.Debug, "DECConnector", string.Format("ChannelConnected\t{0};\t{1};",
                            strVoiceID,
                            strChannelID));
                        lock (mChanStateLocker)
                        {
                            ...
                            if (chanState != null)
                            {
                                chanState.ConnectState = ConnectState.Connected;
                                DoChanStateChangedMessage(chanState);
                            }
                        }
                        break;
                    case DECMessageHelper.MSG_VOC_CHANNELDISCONNECTED:
                        strVoiceID = mDECMessageHelper.GetVoiceIDValue(strData);
                        strChannelID = mDECMessageHelper.GetChannelIDValue(strData);
                        OnDebug(LogMode.Debug, "DECConnector", string.Format("ChannelDisConnected\t{0};\t{1};",
                            strVoiceID,
                            strChannelID));
                        lock (mChanStateLocker)
                        {
                            ...
                            if (chanState != null)
                            {
                                chanState.ConnectState = ConnectState.DisConnected;
                                DoChanStateChangedMessage(chanState);
                            }
                        }
                        break;
                    ...
                    #region 处理RecoverServer发布的消息

                    case DECMessageHelper.MSG_RCV_RECOVERFINISH:
                        SetRecoverFinish(strData);
                        break;
        ...
        #region 设置录音恢复策略的执行状态

        private void SetRecoverFinish(string strData)
        {
            try
            {
                if (mDECMessageHelper == null) { return; }
                string strTaskID = mDECMessageHelper.GetTaskIDValue(strData);
                int flag = 2;
                if (mSession == null
                   || mSession.DatabaseInfo == null)
                {
                    OnDebug(LogMode.Error, string.Format("DatabaseInfo is null"));
                    return;
                }
                DatabaseInfo dbInfo = mSession.DatabaseInfo;
                string strConn = mSession.DBConnectionString;
                ...
                {
                    case 2:
                        strSql = string.Format("SELECT * FROM T_21_011_{0} ORDER BY C001", rentToken);
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        OnDebug(LogMode.Error, "SetRecoverFinish", string.Format("DBType invalid."));
                        return;
                }
                if (objConn == null || objAdapter == null || objCmdBuilder == null)
                {
                    OnDebug(LogMode.Error, "SetRecoverFinish", string.Format("Db object is null."));
                    return;
                }
                ...
                try
                {
                    ...
                    DataRow dr = objDataSet.Tables[0].Select(string.Format("C001 = {0}", strTaskID)).FirstOrDefault();
                    if (dr != null)
                    {
                        dr["C008"] = flag;
                    }
                    ...
                    {
                        OnDebug(LogMode.Error, "SetRecoverFinish", string.Format("TaskID not exist.\t{0}", strTaskID));
                    }
                ...
                {
                    OnDebug(LogMode.Error, "SetRecoverFinish", string.Format("Fail.\t{0}", ex.Message));
                }
                finally
                {
                ...
                OnDebug(LogMode.Info, "SetRecoverFinish", string.Format("End.\t{0}", strTaskID));
            }
            catch (Exception ex)
            {
                OnDebug(LogMode.Error, "SetRecoverFlag", string.Format("Fail.\t{0}", ex.Message));
            }
        }
