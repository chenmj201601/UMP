    ...
    public partial class UMPService02 : ServiceBase
    {
        ...
        public void TestStart()
        {
            try
            {
                ...
                UMPService02 Serverce02 = new UMPService02();
                OnDebug(LogMode.Info, string.Format("Server02 bebug starting..."));
                IStrVerificationCode102 = CreateVerificationCode(EncryptionAndDecryption.UMPKeyAndIVType.M102);
                Serverce02.ReadDatabaseConnectionProfile();
                Serverce02.LogicPartitionDataMain();

            }
            catch (Exception ex)
            {
                OnDebug(LogMode.Error, string.Format("Server02 start fail.\t{0}", ex.Message));
            }
        }
        ...
        {
            CreateFileLog();
            WriteLog(LogMode.Info, string.Format("Service02 starting..."));
                ...
                while (IBoolCanContinue)
                {
                    ...
                    foreach (DataRow LDataRowSingle in LDatabaseReturn.DataSetReturn.Tables[0].Rows)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            WriteEntryLog(
                                string.Format(
                                    "Move21000221001\t Write data to t_21_001 fail.\tColumnName:{0};\tColumnData:{1};\tC000:{2}\t{3}",
                                    LStrColumnName, LStrColumnData, LStrC000, ex), EventLogEntryType.Error);
                        }
                        finally
                        {
        ...
        private void LogicPartitionDataMain()
        {
            ...
            try
            {
                while (IBoolCanContinue)
                {
                    ...
                    //获取所有需要转移的数据表 T_21_001 T_11_901
        ...
        #region 将所有租户表中的数据库转移到逻辑分区表中   ADataTableRentList=00000 ADataTableMovedTables=T_21_001/T_11_901
        ...
        private void MoverEveryRentData2LogicPartitionTable(string AStrRentToken, DataRow ADataRowSourceTable, DataTable AExistLogicPartitionTables)
        {
            ...
            try
            {
                ...
                foreach (DataRow LDataRowSingleTableName in AExistLogicPartitionTables.Rows)
 
                { LListStrExistTables.Add(LDataRowSingleTableName[0].ToString()); }
        ...
        private void UMPServiceLog_EntryWritten(object sender, EntryWrittenEventArgs e)
        {
