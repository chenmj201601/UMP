    ...
    public class MediaOperation : NetSession
    {
        ...
        private void DownloadRecordFile(RequestMessage request)
        {
            ...
            try
            {
                ...
                strPassword = DecryptString(strPassword);       //解密
        ...
        private void DownloadRecordFileNas(RequestMessage request)
        {
            ...
            try
            {
                ...
                string fileName = string.Format("{0}_{1}", recordRef, SessionID);
                fileName = GetBase64String(fileName);
                fileName = string.Format("{0}.{1}", fileName, strExt);
                string path = Path.Combine(MediaDataDir, fileName);
                //文件已经存在，无需重复下载
                if (File.Exists(path))
                {
                ...
                config.Method = 11;     //ftp方式
                config.Host = strAddress;
                config.Port = port;
                config.IsAnonymous = false;
                config.LoginName = strUser;
                config.Password = strPassword;
                config.RequestPath = sourcePath;
        ...
        private void DownloadRecordFileNas2(RequestMessage request)
        {
            int command = request.Command;
            ReturnMessage retMessage = new ReturnMessage();
            retMessage.Result = true;
            ...
            try
            {
                 ...
                 * 0：下载方式
                 * 1：服务器地址或共享目录地址
                 * 2：端口（共享目录或NFS无需端口，可以指定为空或0）
                 * 3：登录名
                 * 4：登录密码（登录名和登录密码可以指定为空）
                 * 5：文件路径（相对于主目录的路径）
                 * 6：RecordReference C002
                 * 7：MediaType （文件类型，默认为 1 .wav 文件）
                 * 
                 * 说明：
                 * 
                 * 下载方式：
                ...
                string strUser = listParams[3];
                string strPassword = listParams[4];
                string sourcePath = listParams[5];
                string recordRef = listParams[6];
                string strMediaType = listParams[7];

                string strLog =
                    string.Format(
                        "Args\tMethod:{0};Host:{1};Port:{2};User:{3};Password:***;SourcePath:{4};RecordRef:{5};MediaType:{6}",
                        strMethod,
                        strAddress,
                        strPort,
                        strUser,
                ...
                strPassword = DecryptString(strPassword);       //密码进行解密
                int intMediaType;
                if (!int.TryParse(strMediaType, out intMediaType)
                    || intMediaType < 0)
                {
                    SendErrorMessage(retMessage, Defines.RET_PARAM_INVALID,
                       string.Format("MediaType invalid.\t{0}", strMediaType));
                    return;
                }
                ...
                string strTempDriver = "Z:";
                if (ConfigInfo != null)
                {
                    var setting =
                        ConfigInfo.ListSettings.FirstOrDefault(s => s.Key == Service03Consts.GS_S03_TEMP_DRIVER);
                    if (setting != null)
                    {
                        strTempDriver = setting.Value;
                    }
                }
                ...
                else if (intMethod == 10)
                {
                    ...
                    string strSharePath = string.Format("{0}", strAddress);
                    string strCommand = string.Format("net use {0} {1} /User:{2} {3} /PERSISTENT:YES",
                        strTempDriver,
                        strSharePath,
                        strUser,
                        strPassword);
                    LogInfo(command, string.Format("Bat command:{0}", strCommand));
                    string strBat = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                        string.Format("Use_{0}.bat", SessionID));
                    if (File.Exists(strBat))
                    {
                        ...
                        {
                            LogError(command, string.Format("Delete file fail.\t{0}\t{1}", strBat, ex.Message));
                        }
                    }
                    string[] listCommands = new string[1];
                    listCommands[0] = strCommand;
                    File.WriteAllLines(strBat, listCommands);
                    OperationReturn optReturn = ExecuteBat(strBat);
                    if (!optReturn.Result)
                    {
                        SendErrorMessage(retMessage, optReturn);
                        return;
                    ...
                    LogInfo(command, string.Format("Use end."));
                    Thread.Sleep(100);
                    sourcePath = sourcePath.TrimStart('\\').TrimStart('/');
                    string strSource = Path.Combine(strTempDriver + "\\", sourcePath);
                    LogInfo(command, string.Format("Begin copy file from {0} to {1}", strSource, path));
                    File.Copy(strSource, path, true);
                    LogInfo(command, string.Format("Copy end."));
                    Thread.Sleep(100);
                    strCommand = string.Format("net use {0} /del", strTempDriver);
                    LogInfo(command, string.Format("Bat command:{0}", strCommand));
                    strBat = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                         string.Format("Delete_{0}.bat", SessionID));
                    if (File.Exists(strBat))
                    {
                        try
                        {
                    ...
                    LogInfo(command, string.Format("Use delete end."));

                    #endregion

                }
                else if (intMethod == 11)
                {

                    #region NFS方式下载

                    string strSharePath = string.Format("{0}", strAddress);
                    string strCommand = string.Format("mount {0} {1}",
                        strSharePath,
                        strTempDriver);
                    LogInfo(command, string.Format("Bat command:{0}", strCommand));
                    string strBat = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                        string.Format("mount_{0}.bat", SessionID));
                    if (File.Exists(strBat))
                    {
                        File.Delete(strBat);
                    }
                    ...
                    LogInfo(command, string.Format("mount end."));
                    Thread.Sleep(100);
                    sourcePath = sourcePath.TrimStart('\\').TrimStart('/');
                    string strSource = Path.Combine(strTempDriver + "\\", sourcePath);
                    LogInfo(command, string.Format("Begin copy file from {0} to {1}", strSource, path));
                    ...
                    LogInfo(command, string.Format("Copy end."));
                    Thread.Sleep(100);
                    strCommand = string.Format("umount -f {0}", strTempDriver);
                    LogInfo(command, string.Format("Bat command:{0}", strCommand));
                    strBat = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                         string.Format("umount_{0}.bat", SessionID));
                    if (File.Exists(strBat))
                    {
                        try
                        {
                    ...
                    LogInfo(command, string.Format("umount end."));
                ...
                {
                    SendErrorMessage(retMessage, Defines.RET_PARAM_INVALID,
                          string.Format("Method invalid.\t{0}", intMethod));
                    return;
                }
        ...
        private void ConvertWaveFormat(RequestMessage request)
        {
            ...
            try
            {
                ...
                //2、转换工具，1：FileConvert；2：Lame；3：ffmpeg；4：VOX2PCM
                if (listParams == null || listParams.Count < 1)
                {
                    SendErrorMessage(retMessage, Defines.RET_PARAM_INVALID,
                       string.Format("ListData is null or count invalid"));
                ...
                {
                    strExePath = Path.Combine(RootDir, string.Format("{0}\\VOX2PCM.exe", ConstValue.TEMP_DIR_MEDIAUTILS));
                }
                else
                {
                ...
                {
                    string strFileName = strSourceFile.Substring(0, strSourceFile.LastIndexOf('.'));
                    strTargetFile = string.Format("F_{0}.{1}", strFileName, strExt);
                }
                string strTargetPath = Path.Combine(MediaDataDir, strTargetFile);
                if (File.Exists(strTargetPath))
                {
                ...
                {
                    strArgs = string.Format(" \"{0}\" \"{1}\"", strSourcePath, strTargetPath);
                }
                else
                {
        ...
        protected override void DoClientCommand(RequestMessage request)
        {
            ...
            switch (request.Command)
            {
                ...
                case (int)Service03Command.DownloadRecordFileNas:
                    ...
                    break;
                case (int)Service03Command.DownloadRecordFileNas2:
                    DownloadRecordFileNas2(request);
                    break;
