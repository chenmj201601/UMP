...
//        All rights reserved
//        guid1:                    629fe55f-4428-448e-96a9-536f01f78027
//        CLR Version:              4.0.30319.18063
//        Name:                     ZipOutputStream
//        Computer:                 CHARLEY-PC
//        Organization:             VoiceCyber
//        Namespace:                VoiceCyber.SharpZips.Zip
//        File Name:                ZipOutputStream
//
//        created by Charley at 2015/7/22 16:49:18
...
using VoiceCyber.SharpZips.Zip.Compression;
using VoiceCyber.SharpZips.Zip.Compression.Streams;
...
namespace VoiceCyber.SharpZips.Zip
{
    ...
    public class ZipOutputStream : DeflaterOutputStream
    {
        ...
        /// <param name="intOffset">The offset of the first byte to write.</param>
        ...
        public override void Write(byte[] buffer, int intOffset, int count)
        {
            if (curEntry == null)
            {
                throw new InvalidOperationException("No open entry.");
...
#else
                throw new ArgumentOutOfRangeException("intOffset", "Cannot be negative");
#endif
            }
            ...
            if ((buffer.Length - intOffset) < count)
            {
                throw new ArgumentException("Invalid offset/count combination");
            }

            crc.Update(buffer, intOffset, count);
            size += count;

            switch (curMethod)
            {
                case CompressionMethod.Deflated:
                    base.Write(buffer, intOffset, count);
                    break;

                case CompressionMethod.Stored:
                    if (Password != null)
                    {
                        CopyAndEncrypt(buffer, intOffset, count);
                    }
                    else
                    {
                        baseOutputStream_.Write(buffer, intOffset, count);
                    }
                    break;
            }
        }

        void CopyAndEncrypt(byte[] buffer, int intOffset, int count)
        {
