    ...
    public partial class App
    {
        ...
        private void CheckWebProtocol()
        {
            try
            {
                ...
                try
                {
                    ...
                    webRequest.Code = (int)S1200Codes.CheckWebProtocol;
                    webRequest.ListData.Add(protocol);
                    ...
                    if (!webReturn.Result)
                    {
                        WriteLog("CheckWebProtocol", string.Format("WSFail.\t{0}\t{1}", webReturn.Code, webReturn.Message));
                        return;
                    }
        ...
        private void GetSettedDefaultLang()
        {
            try
            {
                ...
                doc.Load(path);
                var setNode = doc.SelectSingleNode("UMPSetted");
                if (setNode == null)
                {
