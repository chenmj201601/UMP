    ...
    public partial class Shell
    {
        ...
        private void LoadUserParameters()
        {
            try
            {
                ...
                webRequest.ListData.Add(string.Format("{0}{1}{2}", ConstValue.UP_GROUP_LANG, ConstValue.SPLITER_CHAR,
                    ConstValue.UP_GROUP_PAGE));
        ...
        private void LoadLogicalPartTableInfos()
        {
            try
            {
                ...
                App.WriteLog("LoadPartTables", string.Format("Load end.\t{0}", count));
            }
            catch (Exception ex)
            {
                ShowException(ex.Message);
        ...
        private void DoLoginSystem(WebRequest webRequest)
        {
            try
            {
                ...
                worker.RunWorkerCompleted += (s, re) =>
                {
                    ...
                    var up = mListUserParameters.FirstOrDefault(p => p.ParamID == ConstValue.UP_DEFAULT_LANG);
                    if (up != null)
                    {
                        string strLangID = up.ParamValue;
                        int intLangID;
                        int oldLangID = App.Session.LangTypeID;
                        if (int.TryParse(strLangID, out intLangID)
                            && intLangID > 0
                            && intLangID != oldLangID)
                        {
                            App.WriteLog("LoginSystem",
                                string.Format("Begin change language from {0} to {1}", oldLangID, intLangID));

                            var langInfo = App.Session.SupportLangTypes.FirstOrDefault(l => l.LangID == intLangID);
                            if (langInfo != null)
                            {

                                App.Session.LangTypeInfo = langInfo;
                                App.Session.LangTypeID = langInfo.LangID;
                                ...
                                webRequest.Data = langInfo.LangID.ToString();
        ...
        private void DoChangeLanguage(WebRequest webRequest)
        {
            try
            {
                ...
                #region 将当前语言保存到数据库中，以便记住用户切换的语言

                if (!mIsLogined) { return; }
                BackgroundWorker worker = new BackgroundWorker();
                worker.DoWork += (s, de) =>
                {
                    try
                    {
                        ...
                        paramInfo.UserID = App.Session.UserInfo.UserID;
                        paramInfo.ParamID = ConstValue.UP_DEFAULT_LANG;
                        paramInfo.GroupID = ConstValue.UP_GROUP_LANG;
                        paramInfo.DataType = DBDataType.Int;
                        paramInfo.SortID = 0;
                        paramInfo.ParamValue = langID.ToString();
                        OperationReturn optReturn = XMLHelper.SeriallizeObject(paramInfo);
                        if (!optReturn.Result)
                        {
                            App.WriteLog("SaveUserLang",
                    ...
                    {
                        App.WriteLog("SaveUserLang", string.Format("Fail.\t{0}", ex.Message));
                    }
                };
                worker.RunWorkerCompleted += (s, re) =>
                {
                    ...
                    App.WriteLog("SaveUserLang", string.Format("End.\t{0}", langID));
                };
                worker.RunWorkerAsync();
