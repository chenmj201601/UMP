...
namespace Wcf12001
{
    ...
    public class Service12001 : IService12001
    {
            ...
            try
            {
                ...
                switch (webRequest.Code)
                {
                    ...
                    case (int)S1200Codes.GetAppConfigList:
                        ...
                        webReturn.ListData = optReturn.Data as List<string>;
                        break;
                    case (int)S1200Codes.CheckWebProtocol:
                        optReturn = CheckWebProtocol(session, webRequest.ListData);
                        if (!optReturn.Result)
                        {
                            webReturn.Result = false;
                            webReturn.Code = optReturn.Code;
        ...
        private OperationReturn CheckWebProtocol(SessionInfo session, List<string> listParams)
        {
            ...
            try
            {
                ...
                string strProtocol = listParams[0];
                string strDir = AppDomain.CurrentDomain.BaseDirectory;
                strDir = strDir.Substring(0, strDir.LastIndexOf("\\"));
                strDir = strDir.Substring(0, strDir.LastIndexOf("\\"));
                strDir = Path.Combine(strDir, "GlobalSettings");
                string strFile = Path.Combine(strDir, "UMP.Server.01.xml");
                if (!File.Exists(strFile))
                {
                    optReturn.Result = false;
                    optReturn.Code = Defines.RET_FILE_NOT_EXIST;
                    optReturn.Message = string.Format("UMP.Server.01.xml not exist");
                    return optReturn;
                }
                XmlDocument doc = new XmlDocument();
                doc.Load(strFile);
                var bindingNode = doc.SelectSingleNode("UMPSetted/IISBindingProtocol");
                if (bindingNode == null)
                {
                    optReturn.Result = false;
                    optReturn.Code = Defines.RET_NOT_EXIST;
                    optReturn.Message = string.Format("UMPSetted/IISBindingProtocol node not exist");
                    return optReturn;
                }
                bool isValid = false;
                for (int i = 0; i < bindingNode.ChildNodes.Count; i++)
                {
                    var bind = bindingNode.ChildNodes[i] as XmlElement;
                    if (bind == null) { continue; }
                    string strActive = bind.GetAttribute("Activated");
                    string strValue = bind.GetAttribute("Protocol");
                    if (strValue.ToLower() == strProtocol.ToLower()
                        && strActive == "1")
                    {
                        isValid = true;
                    }
                ...
                if (!isValid)
                {
                    ...
                    optReturn.Message = string.Format("Protocol invalid.\t{0}", strProtocol);
                    return optReturn;
                }
            }
            catch (Exception ex)
                ...
                switch (dbType)
                {
                    case 2:
                        if (topNum > 0)
                        {
                            ...
                            {
                                if (userID == ConstValue.USER_ADMIN)
                                {
                                    strSql = string.Format("SELECT TOP {0} * FROM T_11_901_{1} ORDER BY C008 DESC",
                                        topNum,
                                        strRentToken);
                                }
                                else
                                {
                                    strSql =
                                        string.Format(
                                            "SELECT TOP {0} * FROM T_11_901_{1} WHERE C005 IN (SELECT C004 FROM T_11_201_{1} WHERE C003 = {2}) ORDER BY C008 DESC",
                                            topNum,
                                            strRentToken,
                                            userID);
                                }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (userID == ConstValue.USER_ADMIN)
                                {
                                    strSql = string.Format("SELECT * FROM T_11_901_{0} ORDER BY C008 DESC", strRentToken);
                                }
                                else
                                {
                                    strSql =
                                        string.Format(
                                            "SELECT * FROM T_11_901_{0} C005 IN (SELECT C004 FROM T_11_201_{0} WHERE C003 = {1}) ORDER BY C008 DESC",
                                            strRentToken, userID);
                                }
                            }
                        }
                        ...
                        if (topNum > 0)
                        {
                            ...
                            else
                            {
                                if (userID == ConstValue.USER_ADMIN)
                                {
                                    strSql =
                                        string.Format(
                                            "SELECT * FROM (SELECT * FROM T_11_901_{1} ORDER BY C008 DESC) WHERE ROWNUM < = {0}",
                                            topNum,
                                            strRentToken);
                                }
                                else
                                {
                                    strSql =
                                        string.Format(
                                            "SELECT * FROM (SELECT * FROM T_11_901_{1} WHERE C005 IN (SELECT C004 FROM T_11_201_{1} WHERE C003 = {2}) ORDER BY C008 DESC) WHERE ROWNUM < = {0}",
                                            topNum,
                                            strRentToken,
                                            userID);
                                }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    strSql =
                                        string.Format(
                                            "SELECT * FROM T_11_901_{0} WHERE C005 IN (SELECT C004 FROM T_11_201_{0} WHERE C003 = {1}) ORDER BY C008 DESC",
                                            strRentToken, userID);
                                }
                            }
                        }
        ...
        private OperationReturn GetSupportLangTypeList(SessionInfo session, List<string> listParams)
        {
            ...
            try
            {
                ...
                {
                    case 2:
                        strSql = string.Format("SELECT * FROM T_00_004 ORDER BY C002");
                        optReturn = MssqlOperation.GetDataSet(strConString, strSql);
                        break;
                    case 3:
                        strSql = string.Format("SELECT * FROM T_00_004 ORDER BY C002");
        ...
        private OperationReturn GetUserRoleList(SessionInfo session, List<string> listParams)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    bool isActive = dr["C005"].ToString() == "1";
                    if (!isActive) { continue; }
                    bool isDelete = dr["C006"].ToString() == "1";
                    if (isDelete) { continue; }

                    DateTime now = DateTime.Now.ToUniversalTime();
                    string strValue = dr["C008"].ToString();
                    strValue = DecryptString02(strValue);       //启用时间
                    DateTime dtValue;
                    if (DateTime.TryParse(strValue, out dtValue))
                    {
                        if (dtValue > now) { continue; }
                    }
                    strValue = dr["C009"].ToString();
                    strValue = DecryptString02(strValue);       //过期时间
                    if (DateTime.TryParse(strValue, out dtValue))
                    {
                        if (dtValue < now) { continue; }
                    }
