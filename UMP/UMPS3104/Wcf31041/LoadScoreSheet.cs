...
namespace Wcf31041
{
    public partial class Service31041
    {
                ...
                switch (session.DBType)
                {
                    case 2:
                        strSql = string.Format(
                            "SELECT T3102.*,T3109.C005 AS NA FROM T_31_002_{0} T3102 LEFT JOIN T_31_009_{0} T3109 ON T3102.C002=T3109.C002  WHERE T3102.C003 = {1} AND T3102.C004 = {2}{3} ",
                                rentToken,scoreSheet.ID,parent.ID,scoreID);
                        optReturn = MssqlOperation.GetDataSet(session.DBConnectionString, strSql);
                        if (!optReturn.Result)
                        {
                            return optReturn;
