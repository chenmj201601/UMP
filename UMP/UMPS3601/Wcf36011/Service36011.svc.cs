    ...
    public class Service36011 : IService36011
    {
        ...
        private OperationReturn OptDeleteCategory(SessionInfo session, List<string> listParams)
        {
            ...
            try
            {
                ...
                OperationReturn optReturnTemp = DeleteChildrenNodes(session, papersCategoryParam.LongNum.ToString(), 0);
        ...
        private OperationReturn DeleteChildrenNodes(SessionInfo session, string strNodeID, int iDepth)
        {
            OperationReturn optReturn = new OperationReturn();
            string strSql1 = string.Format("SELECT * FROM T_36_021_{0} WHERE C003='{1}'", session.RentInfo.Token, ParsedString(strNodeID));
            DataSet objDataSet;
            switch (session.DBType)
            {
                case 2:
            ...
            DataSet objDataSet1;
            string strSql2 = string.Format("SELECT * FROM T_36_022_{0} WHERE C002='{1}'", session.RentInfo.Token, ParsedString(strNodeID));
            switch (session.DBType)
            {
                case 2:
            ...
            {
                iDepth++;
                string strSqlTemp = string.Format("DELETE FROM T_36_021_{0} WHERE C001='{1}'", session.RentInfo.Token, ParsedString(strNodeID));
            ...
            for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
            {
                ...
                iDepth++;
                OperationReturn optReturnTemp = DeleteChildrenNodes(session, strID, iDepth);
                if (optReturnTemp.Message == S3601Consts.HadUse)
                {
                    optReturn.Message = S3601Consts.HadUse;
                    return optReturn;
                }
                string strSql3 = string.Format("SELECT * FROM T_36_022_{0} WHERE C002='{1}'", session.RentInfo.Token, strID);
                DataSet objDataSet2;
                switch (session.DBType)
                {
                    case 2:
                ...
                {
                    iDepth++;
                    string strSqlTemp = string.Format("DELETE FROM T_36_021_{0} WHERE C001='{1}'", session.RentInfo.Token, strID);
                    _mListDeleteInfo.Add(strSqlTemp);
                }
            }
            return optReturn;
