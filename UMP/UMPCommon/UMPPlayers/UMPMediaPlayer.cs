...
using OperationReturn = VoiceCyber.Common.OperationReturn;
...
namespace VoiceCyber.UMP.Controls.Players
{
    ...
    public class UMPMediaPlayer : Control
    {
        ...
        public UMPMediaPlayer()
        {
            ...
            mListVideoUrls = new List<string>();
            ...
            if (mWaveStream != null)
            {
                ...
                Dispatcher.Invoke(new Action(() =>
                {
                    ...
                    try
                    {
                        ...
                        //LbRate = Converter.Second2Time(mScreenPlayedLength);    //测试，在播放速度上显示录屏播放位置
        ...
        private OperationReturn PlayVClogVedio()
        {
            ...
            try
            {
                ...
                //先清空文件列表
                intReturn = ScreenMPInterop.VLSMonClearFile();
                ShowDebugMessage(string.Format("ClearFileReturn:{0}", intReturn));

                //将录屏文件传给播放dll
                for (int i = 0; i < mListVideoUrls.Count; i++)
                {
                    string strVideoUrl = mListVideoUrls[i];
                    intReturn = ScreenMPInterop.VLSMonAddFile(strVideoUrl, 1);
                    ShowDebugMessage(string.Format("AddFileReturn:{0}", intReturn));
                }

                //播放录屏数据
                intReturn = ScreenMPInterop.VLSMonPlayVls(null, handle);
                ...
                if (intReturn == 0)
                {
                    ...
                    ////设置标题和获取总时长于 2016/10/10 屏蔽调，不再由录屏文件获取时长了     by Charley at 2016/10/10
