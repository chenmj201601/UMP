    ...
    public class QutityCountFuncs
    {
        ...
        public static OperationReturn CreateSql(SessionInfo session, List<string> lstParam)
        {
            ...
            try
            {
                ...
                {
                    case 2:
                        strSql = string.Format("SELECT *   FROM T_31_008_{0} where   C006 between '{1}' and '{2}'", strToken, startTime, EndTime);
                        break;
                    case 3:
                        strSql = string.Format("SELECT *  FROM T_31_008_{0} where " +
                                                        " C006 >= to_date('{1}','yyyy-mm-dd,hh24:mi:ss')  " +
                                                         "and C006 <= to_date('{2}','yyyy-mm-dd,hh24:mi:ss')", strToken, startTime, EndTime);
                        break;
                }
