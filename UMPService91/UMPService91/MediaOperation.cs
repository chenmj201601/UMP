...
//        All rights reserved
//        GUID1:                    652d07e4-084d-46c2-8593-fcfc496d3630
...
//
//        Created by Charley at 2016/8/18 10:22:19
...
namespace UMPService91
{
    public class MediaOperation : NetSession
    {
        ...
        {
            int command = (int)Service91Command.IsaNotifyPosition;
            try
            {
                OperationReturn optReturn;
                var msg = e.Data;
                   ...
                   && json["frambuffer"]["position"] != null)
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        if (json["frambuffer"]["pixels"][count - 1]["pixelImg"] != null)
                        {
                            ...
                            //裁剪图片
                            optReturn = CropScreenImage(strImagFile);
                            if (!optReturn.Result)
                            {
                                LogError(command,
                                  string.Format("Crop image fail.\t{0}\t{1}", optReturn.Code, optReturn.Message));
                                return;
                            }
                    ...
                    notMessage.Command = (int)Service91Command.IsaNotifyPosition;
        ...
        private OperationReturn CropScreenImage(string strFile)
        {
            OperationReturn optReturn = new OperationReturn();
            optReturn.Result = true;
            optReturn.Code = 0;
            try
            {
                int screenMode = 0;     //0：未知；1：单屏；2：双屏；3：三屏；4：四屏
                ...
                {
                    var setting =
                        ConfigInfo.ListSettings.FirstOrDefault(s => s.Key == Service91Consts.GS_KEY_S91_ISA_SCR_MODE);
                    if (setting != null)
                    {
                        int intValue;
                ...
                string strMediaDataDir = MediaDataDir;
                string strSourcePath = Path.Combine(strMediaDataDir, strFile);
                if (!File.Exists(strSourcePath))
                {
                    optReturn.Result = false;
                    optReturn.Code = Defines.RET_FILE_NOT_EXIST;
                    optReturn.Message = string.Format("Source image file not exist.\t{0}", strSourcePath);
                    return optReturn;
                }
                BitmapImage bitmap = new BitmapImage();
                bitmap.BeginInit();
                bitmap.UriSource = new Uri(strSourcePath, UriKind.RelativeOrAbsolute);
                bitmap.EndInit();
                int width = bitmap.PixelWidth;
                int height = bitmap.PixelHeight;
                string strCroppedFiles = string.Empty;
                ...
                switch (screenMode)
                {
                    case 0:
                    case 1:
                        strCroppedFiles = strFile;
                        break;
                    case 4:
                        strCroppedFiles += string.Format("{0};", strFile);

                        strTempFile = string.Format("1_{0}", strFile);
                        strTempPath = Path.Combine(strMediaDataDir, strTempFile);
                        croppedBitmap = new CroppedBitmap(bitmap, new Int32Rect(0, 0, width / 4, height));
                        encoder = new PngBitmapEncoder();
                        encoder.Frames.Add(BitmapFrame.Create(croppedBitmap));
                        fs = new FileStream(strTempPath, FileMode.Create, FileAccess.ReadWrite);
                        encoder.Save(fs);
                        fs.Close();
                        strCroppedFiles += string.Format("{0};", strTempFile);

                        strTempFile = string.Format("2_{0}", strFile);
                        strTempPath = Path.Combine(strMediaDataDir, strTempFile);
                        croppedBitmap = new CroppedBitmap(bitmap, new Int32Rect(width / 4, 0, width / 4, height));
                        encoder = new PngBitmapEncoder();
                        encoder.Frames.Add(BitmapFrame.Create(croppedBitmap));
                        fs = new FileStream(strTempPath, FileMode.Create, FileAccess.ReadWrite);
                        encoder.Save(fs);
                        ...
                        strTempFile = string.Format("3_{0}", strFile);
                        strTempPath = Path.Combine(strMediaDataDir, strTempFile);
                        croppedBitmap = new CroppedBitmap(bitmap, new Int32Rect(width / 2, 0, width / 4, height));
                        encoder = new PngBitmapEncoder();
                        encoder.Frames.Add(BitmapFrame.Create(croppedBitmap));
                        fs = new FileStream(strTempPath, FileMode.Create, FileAccess.ReadWrite);
                        encoder.Save(fs);
                        ...
                        strTempFile = string.Format("4_{0}", strFile);
                        strTempPath = Path.Combine(strMediaDataDir, strTempFile);
                        croppedBitmap = new CroppedBitmap(bitmap, new Int32Rect(width * 3 / 4, 0, width / 4, height));
                        encoder = new PngBitmapEncoder();
                        encoder.Frames.Add(BitmapFrame.Create(croppedBitmap));
                        fs = new FileStream(strTempPath, FileMode.Create, FileAccess.ReadWrite);
                        encoder.Save(fs);
                    ...
                    default:
                        ...
                        optReturn.Message = string.Format("ScreenMode invalid");
                        return optReturn;
                }
                optReturn.Data = strCroppedFiles;
            }
            catch (Exception ex)
            {
                optReturn.Result = false;
        ...
        protected override void DoClientCommand(RequestMessage request)
        {
            ...
            {
                case (int)Service91Command.DownloadRecordFile:
                    DownloadRecordFile(request);
                    break;
                case (int)Service91Command.DownloadRecordFileNas:
                    DownloadRecordFileNas(request);
                    break;
                case (int)Service91Command.DownloadRecord:
                    DownloadRecord(request);
                    break;
                case (int)Service91Command.ConvertWaveFormat:
                    ConvertWaveFormat(request);
                    break;
                case (int)Service91Command.DecryptRecordFile:
                    DecryptRecordFile(request);
                    break;
                case (int)Service91Command.OriginalDecryptFile:
                    OriginalDecryptFile(request);
                    break;
                case (int)Service91Command.EncryptRecordFile:
                    EncryptRecordFile(request);
                    break;
                case (int)Service91Command.IsaStart:
                    IsaStart(request);
                    break;
                case (int)Service91Command.IsaStop:
                    IsaStop(request);
                    break;
                case (int)Service91Command.IsaBehavior:
                    IsaBehavior(request);
                    break;
                case (int)Service91Command.UploadFile:
                    UploadFile(request);
                    break;
            }
        }
        ...
        protected override string ParseCommand(int command)
        {
            ...
            {
                str = ((Service91Command)command).ToString();
            }
            return str;
        }
        ...
        private OperationReturn ExecuteExe(string exePath, string arguments)
        {
            ...
            try
            {
                ...
                if (intRet != 0)
                {
                    ...
                    {
                        optReturn.Result = false;
                        optReturn.Code = Service91Consts.DECRYPT_PASSWORD_ERROR;
