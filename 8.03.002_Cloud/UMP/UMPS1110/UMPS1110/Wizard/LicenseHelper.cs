...
namespace Wcf11101
{
    public class LicenseHelper : IEncryptable
    {
        ...
        {
            //return source;
            return ClientAESEncryption.DecryptString(source, (EncryptionMode)mode, Encoding.ASCII);
        }
        ...
        {
            //return source;
            return ClientAESEncryption.EncryptString(source, (EncryptionMode)mode, Encoding.ASCII);
        }
        ...
        {
            return ClientAESEncryption.DecryptString(source, (EncryptionMode)mode, encoding);
        }

        public string EncryptString(string source, int mode, Encoding encoding)
        {
            return ClientAESEncryption.EncryptString(source, (EncryptionMode)mode, encoding);
        }

        public byte[] DecryptBytes(byte[] source, int mode)
        {
            return ClientAESEncryption.DecryptBytes(source, (EncryptionMode)mode);
        }
        ...
        {
            return ClientAESEncryption.EncryptBytes(source, (EncryptionMode)mode);
        }
