   ...
   public  class XMLOperator
    {
        ...
        public XMLOperator(XmlDocument xml)
        {
            try
            {
                XmlDoc = xml;
       ...
       public bool UpdateAttrib(XmlNode node,string Attrib, string AttribContent)
        {
            try
            {
                //修改属性值
               node.Attributes[Attrib].Value = AttribContent;
                return true;
            }
            catch
            {
        ...
        public bool InsertAttrib(XmlNode node, string Attrib, string AttribContent)
        {
            try
            {
                XmlElement objNode = (XmlElement)node; //强制转化成XmlElement对象
                objNode.SetAttribute(Attrib, AttribContent); //XmlElement对象添加属性方法   
                return true;
            }
            catch
        ...
        /// <param name="lstAttribs"></param>
        /// <returns></returns>
        public bool InsertAttribs(XmlNode node, List<AttribEntry> lstAttribs)
        {
            try
            {
                XmlElement objNode = (XmlElement)node; //强制转化成XmlElement对象
                for (int i = 0; i < lstAttribs.Count; i++)
                {
                    objNode.SetAttribute(lstAttribs[i].AttribName, lstAttribs[i].AttribContent); //XmlElement对象添加属性方法   
                }
        ...
        /// <param name="lstAttribs"></param>
        /// <returns></returns>
        public bool InsertAttribs(string strNodeName, List<AttribEntry> lstAttribs)
        {
            try
            {
                XmlElement objNode = (XmlElement)XmlDoc.SelectSingleNode(strNodeName);  //强制转化成XmlElement对象
                for (int i = 0; i < lstAttribs.Count; i++)
                {
                    objNode.SetAttribute(lstAttribs[i].AttribName, lstAttribs[i].AttribContent); //XmlElement对象添加属性方法   
                }
        ...
        /// 创建一个子节点 
        /// </summary>
        /// <param name="strNodeName">要创建的节点名</param>
        /// <param name="MainNode">父节点</param>
        /// <returns></returns>
        public XmlNode InsertNode(string strNodeName, XmlNode MainNode)
        {
            try
            {
                XmlNode node = XmlDoc.CreateElement(strNodeName); ;
        ...
        /// 添加一个子节点
        /// </summary>
        /// <param name="MainNode">父节点路径</param>
        /// <param name="ChildNode">子节点名</param>
        /// <returns></returns>
        public XmlNode InsertNode(string MainNode, string ChildNode)
        {
            try
            {
                XmlNode objRootNode = XmlDoc.SelectSingleNode(MainNode); //声明XmlNode对象
