                    ...
                    else
                    {
                        while (lstRecord.Count < LIntNumber)
                        {
                            ...
                            switch (UMPService06.IIntDBType)
                            {
                                case 2:
                                    {
                                        LStrSQL = string.Format("SELECT TOP 1  C001,C002,C003,C006,C009,C010 ,C020 FROM  T_21_998 WHERE   C001>{1} AND C001<={2}  AND C006='{0}'  AND C019='E'  ORDER BY C001 ", LStrVoiceIP, ReferenceNum, LMaxReference);
                                ...
                                case 3:
                                    {
                                        LStrSQL = String.Format("SELECT  C001,C002,C003,C006,C009,C010 ,C020  FROM T_21_998 WHERE ROWNUM = 1  AND C001>{1} AND C001<={2} AND C006='{0}'  AND C019='E'   ORDER BY C001  ", LStrVoiceIP, ReferenceNum, LMaxReference);
                                        FileLog.WriteInfo("ClientMessageG001()", "  StrSQL= " + LStrSQL + " | FromIP:" + mRemoteEndpoint);
                                        optReturn = OracleOperation.GetDataSet(UMPService06.IStrDBConnectProfile, LStrSQL);
                ...
                foreach (ObjectBindingPolicy o in lstAllObjectBindingPolicy)
                {
                    ...
                    if (o.PolicyID == Int64.Parse(LTrueEnyptionKeyDictionary.PolicyID))
                    {
                        ...
                        switch (UMPService06.IIntDBType)
                        {
                            case 2:
                                {
                                    LSelectStr = string.Format("SELECT C001, C002,C004, C005, C007,C008 FROM T_24_005  WHERE C002 ={0} AND C007 >= {1} AND C007 <= {2}  AND C007<={3} AND C008>{4} ", o.PolicyID,
                                        Convert.ToDecimal(LStartRecordTime.ToString("yyyyMMddHHmmss")),
                                        Convert.ToDecimal(LDateTimeQueryKeyTime.ToString("yyyyMMddHHmmss")),
                    ...
                    else 
                    {
                        ...
                        switch (UMPService06.IIntDBType)
                        {
                            case 2:
                                {
                                    LSelectStr = string.Format("SELECT C001, C002,C004, C005, C007,C008 FROM T_24_005  WHERE C002 ={0} AND C007 <= {1}  AND C007<={2} AND C008>{3} ", o.PolicyID,
                                        Convert.ToDecimal(LDateTimeQueryKeyTime.ToString("yyyyMMddHHmmss")),
                                        Convert.ToDecimal(o.DurationEnd.ToString("yyyyMMddHHmmss")),
                                        Convert.ToDecimal(o.DurationBegin.ToString("yyyyMMddHHmmss")));
                                    FileLog.WriteInfo("ClientMessageG003()", "LSelectStr:" + LSelectStr + " |  FromIP:" + RemoteEndpoint);
