    ...
    public class Service44101 : IService44101
    {
            ...
            try
            {
                ...
                for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (intMethod == 0)
                    {
                        ...
                        item.AlarmType = Convert.ToInt32(dr["C004"]);
        ...
        private OperationReturn SaveRegionInfo(SessionInfo session, List<string> listParams)
        {
            ...
            try
            {
                ...
                if (regionID == 0)
                {
                    ...
                    regionInfo.ObjID = Convert.ToInt64(webReturn.Data);
                }
                listReturn.Add(regionInfo.ObjID.ToString());
        ...
        private OperationReturn SaveAlarmMessageInfo(SessionInfo session, List<string> listParams)
        {
            ...
            try
            {
                ...
                //1         Method（模式：0，默认模式，即存在则修改【M】，不存在则新增【A】；1，存在则忽略【I】，不存在则新增【A】；2：存在则删除【D】，不存在则忽略【I】）
                //2         count（数量）
                //3...      AlarmMessageInfo（SerialID为0表示新增）
                //
                if (listParams == null
                   || listParams.Count < 3)
                {
                ...
                for (int i = 0; i < intCount; i++)
                {
                    AlarmMessageInfo agentStateInfo = listAlarmInfos[i];
                    long objID = agentStateInfo.SerialID;
                    if (objID <= 0)
                    {
                        //获取流水号
                        WebRequest webRequest = new WebRequest();
                        ...
                        agentStateInfo.SerialID = Convert.ToInt64(webReturn.Data);
                    }
                }
                List<string> listReturn = new List<string>();
