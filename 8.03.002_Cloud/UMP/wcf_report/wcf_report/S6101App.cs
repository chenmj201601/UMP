...
namespace UMPS6101
{
    public class S6101App : UMPApp
    {
        ...
        protected override void InitSessionInfo()
        {
            ...
            userInfo.UserID = 1021609181600000002;
            Session.UserID = 1021609181600000002;
            ...
            //dbInfo.Port = 1521;
            //dbInfo.DBName = "Orcl";
            //dbInfo.LoginName = "UMP6";
            //dbInfo.Password = "ump6";
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (ds_RecorderSorce != null && ds_RecorderSorce.Tables.Count != 0)
                    {
                        ...
                        for (int k = 0; k < RecorderID.Count; k++)
                        {
                            ...
                            if (Mark != -1)
                            {
                                DT_Final.ImportRow(RecorderRows[Mark]); dtM.ImportRow(RecorderRows[Mark]);
                            }
                        }
                        DataSetMark.Tables.Add(dtM);
        ...
        public DataSet GetR22DataSet(string StrQ, int t)
        {
            ...
            do
            {
                ...
                if (Session.DBType == 2)
                {
                    sql = string.Format("SELECT TOP {0} * " +
                        "FROM T_31_042_{1}  " +
                    "WHERE C001>{2} AND C009=1 AND C017='K' AND C011 >={3} AND C011 <={4} {5} ORDER BY C001,C011"
                ...
                else
                {
                    sql = string.Format("SELECT * FROM (SELECT  * "
                        + "FROM T_31_042_{1} WHERE C009=1 AND C017='K' " + StrQ
                        + "AND C011 >= " + ADT.BeginDateTime[t].ToString("yyyyMMddHHmmss") + " AND C011 <= "
                        + ADT.EndDateTime[t].ToString("yyyyMMddHHmmss") + " ORDER BY C001,C019) WHERE C001 >" + AfterRowNum + " AND ROWNUM<={0} ORDER BY C001,C019 DESC ", GetDataNum, Session.RentInfo.Token);
                }
                if (first_time)
                {
                    _ds = GetDataSetFromDB(sql, 100);
            ...
            if (CiShuSum != 0)
            {
                ...
                foreach (DataRow dr in _dt.Rows)
                {
                    string KeyWordID = dr["C006"].ToString();
                    ...
                    if (IsFind == null)
                    {
                        ...
                        newRow["C006"] = dr["C006"].ToString();
                        DataRow[] CountRows = _dt.Select("C006='" + KeyWordID+"'");
            ...
            if (CiShuSum != 0)
            {
                ...
                for (int j = 0; j < ListGroup.Count; j++)
                {
                    ...
                    for (int q = 0; q < ListColName.Count(); q++)
                    {
                        ...
                        {
                            if (ListData[rum]["C006"].ToString() == TListKeyWord[q])
                            {
                                KeyWcount++;
                            }
                        }
        ...
        public DataSet GetR25DataSet(string StrQ, int t, string ColName)
        {
            ...
            if (CiShuSum != 0)
            {
                ...
                for (int j = 0; j < _dt.Rows.Count; j++)
                {
                    ...
                    newRow["C006"] = _dr["C006"];
                    newRow["C011"] = _dr["C011"];
                    double TimeNum = 0;
                    if (double.TryParse(_dr["C004"].ToString(), out TimeNum))
                    {
                        newRow["C004"] = Converter.MilliSecond2Time(TimeNum);
                    }
                    else
                    {
                        newRow["C004"] = _dr["C004"].ToString();
                    }
        ...
        public void GetTenant()
        {
            WebRequest webRequest = new WebRequest();
            webRequest.Session = Session;
            webRequest.Code = (int)Common61011.WebCodes.GetTenant;
            Service61012Client client = new Service61012Client(
                WebHelper.CreateBasicHttpBinding(Session)
                , WebHelper.CreateEndpointAddress(Session.AppServerInfo, "Service61012"));
            WebHelper.SetServiceClient(client);
            ...
            TenantName = webReturn.Data;
            List<DataRow> drTenant = ds_ap.Tables[0].Select(string.Format("C001='{0}'", TenantName)).ToList();
            if (drTenant != null && drTenant.Count != 0)
            {
                TenantName = DecryptString(drTenant[0]["C002"].ToString());
            }
        }
        ...
        public void GetDataSet()
        {
            try
            {
                ...
                ds_keywords = GetDataSetFromDB(sqlKeyWord, 0);
            }
            catch (Exception ex)
            { }
        }//获取相关要用的一些信息
