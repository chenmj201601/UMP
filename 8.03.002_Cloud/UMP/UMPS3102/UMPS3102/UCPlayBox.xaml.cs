    ...
    public partial class UCPlayBox
    {
        ...
        private void PlayRecord()
        {
            try
            {
                if (RecordInfoItem == null)
                {
                    CurrentApp.ShowInfoMessage("上条录音还在下载中，不能播放下一条");
                    return;
                }
                RecordInfo recordInfo = RecordInfoItem.RecordInfo;
                if (recordInfo == null) { return; }
                ...
                if (recordInfo.MediaType == 3)
                {
                    ...
                    PlayRecord(fileName, string.Empty);
                    return;
                }
                ...
                mWorker.DoWork += (s, de) =>
                {
                    try
                    {
                        ...
                        if (relativeRecord != null)
                        {
                            ...
                            {
                                //艺赛旗录屏无需下载，也不用解密转换等操作，直接跳过以下操作
                                relativeName = relativeRecord.IsaRefID;
                            }
                            else
                            {
                                //下载录屏文件到AppServer
                ...
                switch (mediaType)
                {
                    ...
                    case 1:
                        ...
                        else
                        {
                            ...
                            var relativeRecord = RecordInfoItem.ListRelativeInfos.FirstOrDefault();
                            if (relativeRecord != null
                                && !string.IsNullOrEmpty(relativeName))
                            {
                                if (relativeRecord.MediaType == 3)
                                {
                                    ...
                                    CurrentApp.WriteLog("PlayRecord", string.Format("VedioPath:{0}(IsaVedio)", relativeName));
                                    VoicePlayer.IsIsaScreen = true;
