    ...
    public partial class Service31021
    {
        ...
        private OperationReturn GetKeyWordsInfo(SessionInfo session, List<string> listParams)
        {
            ...
            try
            {
                ...
                {
                    case 2:
                        strSql = string.Format("SELECT * FROM T_11_009_{0}  WHERE C000 = '4'  AND C010<>'1' AND C009 IN (SELECT C001 FROM T_11_009_{0} WHERE C000 = 3)",
                            rentToken);
                        optReturn = MssqlOperation.GetDataSet(session.DBConnectionString, strSql);
                        if (!optReturn.Result)
                        {
                ...
                for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    DataRow dr = objDataSet.Tables[0].Rows[i];
                    string strID = dr["C001"].ToString();
                    string strName = dr["C006"].ToString();
                    strName = DecryptString02(strName);
